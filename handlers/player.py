...
async def get_task(message: Message):
    ...
    try:
        ...
        cur.execute("""
            SELECT pt.id as player_task_id, pt.seq_num, pt.status, pt.started_at, t.text
        ...
        if active:
            ...
            if started_at:
                ...
                if minutes < 60:
                    ...
                    # Время вышло — помечаем как timeout
                    cur.execute("""
                        UPDATE player_tasks
        ...
        # Ищем следующее свободное задание
        cur.execute("""
        ...
        if not row:
            await message.answer("🎉 Все задания для твоей команды уже выполнены или просрочены!")
            conn.close()
            return
...
async def get_hint(message: Message):
    ...
    try:
        ...
        if not started_at:
            await message.answer("⚠️ Ошибка: у задания нет времени выдачи.")
            conn.close()
            return
        ...
        if row["status"] == "waiting_answer":
            if minutes < 20:
                ...
                cur.execute("""
                    UPDATE player_tasks SET status='hint1_sent'
                    WHERE id=%s
                """, (row["player_task_id"],))
                conn.commit()
            ...
            if minutes < 40:
                ...
                cur.execute("""
                    UPDATE player_tasks SET status='hint2_sent'
                    WHERE id=%s
                """, (row["player_task_id"],))
                conn.commit()
